...
const events = eventsTxt.trim().split('\n').map(line => {
...
events.forEach(event => {
  eventDivs += `<div class="event-box"><span>${event.title}: </span><span id="${event.title}"></span></div>\n`;
});
...
countdownJs = countdownJs.replace('// This will be populated by the build script', `const events = ${JSON.stringify(events)};`);
